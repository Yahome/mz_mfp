...
4. 列配置: `60px 1fr 180px 120px` (序号/手术名称/手术日期/操作)
5. 添加上移/下移/删除按钮

**不确定内容**:
- ❓ 手术日期字段是否需要DatePicker还是保持Input type="datetime-local"?
- ❓ 是否需要手术编码字段(类似诊断编码)?
...
**任务内容**:
1. 参照DiagnosisGroupCard的改造方式
2. 列配置: `60px 150px 1fr 180px 120px` (序号/操作编码/操作名称/操作日期/操作)
...
**预计工时**: 1.5小时
**依赖**: Phase 2全部完成
**输出文件**: `frontend/src/components/HerbDetailCard.tsx`

**任务内容**:
1. 移除Ant Design Table
2. 实现plane-table Grid布局
3. 列配置: `40px 150px 80px 100px 80px 80px 100px 120px` (序号/药品名称/剂型/剂量/单位/频次/用法/操作)
4. 添加上移/下移/删除按钮
...
**预计工时**: 30分钟
**依赖**: 无(可并行)
**输出文件**: `frontend/src/pages/RecordForm/FeeDisplayComplete.tsx`
...
**预计工时**: 2小时
**依赖**: 无(可并行)
**输出文件**: `frontend/src/pages/RecordForm/BoardingPassHeader.tsx`
