...
## 4. 数据来源（视图）
- `V_EMR_MZ_PAT_MASTER_INDEX.sql`（blh=patient_no）：`JGMC/ZZJGDM/USERNAME/JZKH/XM/XB/CSRQ/HY/GJ/MZ/ZJLB/ZJHM/XZZ/LXDH/GHSJ/BDSJ/JZSJ/JZKS/JZKSDM/JZYS/JZYSZC` 等。
- `V_EMR_MZ_PAGE_FEE.sql`：总费用、自付、42 分项（含医保基金支付→自费=总费用-医保），已完成映射；并新增用药情况判断（用于 `XYSY/ZCYSY/ZYZJSY/CTYPSY/PFKLSY`）。
...
- 用药：
  - 用药情况标识（必填）：`XYSY/ZCYSY/ZYZJSY/CTYPSY/PFKLSY` 优先从 `V_EMR_MZ_PAGE_FEE.sql` 视图预填（基于收费/处方明细判断）；通常不允许人工修改。
  - 中草药明细（条件必填）：`ZCYLB1..40`、`YYTJDM1..40`、`YYTJMC1..40`、`YYJS1..40` 仍需从处方/医嘱视图汇总；`YYTJMC` 建议由 `YYTJDM` 反查自动回填。
- 费用：`V_EMR_MZ_PAGE_FEE.sql` 直填，总额/自费/42 分项只读。
...
- 用药情况标识校验：`XYSY/ZCYSY/ZYZJSY/CTYPSY/PFKLSY` 值域必须符合 RC016（`1` 是、`2` 否）；当前 `V_EMR_MZ_PAGE_FEE.sql` 视图已直接输出 `1/2`，后端可直接透传。
